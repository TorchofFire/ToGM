...
class Rock(object):
	...
	def __init__(self, x, y):
		...
		self.width = 2 * pixScale
		self.height = 4 * pixScale
		self.xSpeed = 0
		self.ySpeed = 0
		self.hitbox = (self.x + (7 * pixScale), self.y + (6 * pixScale), self.width, self.height)#defines hitbox
		self.passing = False
		self.hit = False
	...
	def updateHitbox(self):
		self.hitbox = (self.x + (7 * pixScale), self.y + (6 * pixScale), self.width, self.height)
